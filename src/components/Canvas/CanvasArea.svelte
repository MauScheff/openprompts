<script lang="ts">
  ...
  import type { SceneShape, CircleNode } from '../../lib/scene';

  export let scene: SceneShape[] = $bindable([]);
  export let viewScale = $bindable(1);
  export let viewOffset: [number, number] = $bindable([0, 0]);
  export let editingScene = $bindable(false);
  export let canvas: HTMLCanvasElement | null = $bindable(null);
  ...
  onMount(() => {
    ...
    drawCircle = makeDrawCircle(regl, () => viewScale, () => [viewOffset[0], viewOffset[1]]);
    ...
    canvas.addEventListener('mousedown', (e) => {
      ...
      if (hitShape) {
        ...
        const prevSelected = scene.find((s) => s.type === 'circle' && s.selected) as CircleNode | undefined;
        if (prevSelected && prevSelected !== hitShape && prevSelected.role !== 'output') {
          scene = addEdge(scene, prevSelected, hitShape);
          scene.forEach((s) => { if (s.type === 'circle') s.selected = false; });
          dragging = null;
          return;
        }
        dragging = hitShape;
        ...
        for (let i = scene.length - 1; i >= 0; i--) {
          ...
          if (dist2 <= EDGE_TOLERANCE * EDGE_TOLERANCE) { hitEdge = s; break; }
        }
        if (hitEdge) {
    ...
    window.addEventListener('mouseup', (e) => { if (e.button === 2 || e.button === 0) { isPanning = false; } });
    ...
    window.addEventListener('keydown', (e) => {
      ...
      if (circleIdx !== -1) {
        ...
        if (circle.role === 'input' || circle.role === 'output') { return; }
    ...
    regl.frame(() => {
      ...
      drawEdges(scene, drawLine, drawTriangle, viewScale, viewOffset, selectedColor, highlightColor, canvas!);
      scene.forEach((shape) => {
        if (shape.type === 'circle') {
          if (shape.selected) {
            const outlineFactor = 1.2;
            drawCircle({ center: shape.center, radius: shape.radius * outlineFactor, color: selectedColor });
            drawCircle({ center: shape.center, radius: shape.radius, color: shape.color });
          } else if (shape.highlight) {
            drawCircle({ center: shape.center, radius: shape.radius, color: highlightColor });
          } else {
            drawCircle({ center: shape.center, radius: shape.radius, color: shape.color });
          }
        }
